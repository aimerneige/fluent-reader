...
class FeverConfigsTab extends React.Component {
    constructor(props) {
        ...
        this.save = async () => {
            ...
            if (this.state.existing) {
                ...
                if (this.state.password)
                    configs.apiKey = (0, js_md5_1.md5)(`${configs.username}:${this.state.password}`);
            }
            else {
                configs = {
                    type: 1 /* SyncService.Fever */,
                    endpoint: this.state.endpoint,
                    username: this.state.username,
                    fetchLimit: this.state.fetchLimit,
                    apiKey: (0, js_md5_1.md5)(`${this.state.username}:${this.state.password}`),
                };
        ...
        this.state = {
            existing: configs.type === 1 /* SyncService.Fever */,
    ...
    render() {
        return (React.createElement(React.Fragment, null,
            ...
            React.createElement(react_1.Stack, { horizontalAlign: "center", style: { marginTop: 48 } },
                React.createElement(react_1.Icon, { iconName: "Calories", style: {
                        color: "var(--black)",
                        fontSize: 32,
                        userSelect: "none",
                    } }),
                React.createElement(react_1.Label, { style: { margin: "8px 0 36px" } }, "Fever API"),
...
exports.default = FeverConfigsTab;
