...
function setUtilsListeners(manager) {
    ...
    electron_1.ipcMain.handle("show-error-box", async (_, title, content, copy) => {
        if (manager.hasWindow() && copy != null) {
            const response = await electron_1.dialog.showMessageBox(manager.mainWindow, {
                type: "error",
                title: title,
                message: title,
                detail: content,
            ...
            if (response.response === 1) {
                electron_1.clipboard.writeText(`${title}: ${content}`);
            }
        }
        else {
                ...
                if ((params.hasImageContents ||
                    ...
                    if (params.hasImageContents) {
                        ...
                        electron_1.ipcMain.handleOnce("image-callback", (_, type) => {
                            switch (type) {
                                case 0 /* ImageCallbackTypes.OpenExternal */:
                                case 1 /* ImageCallbackTypes.OpenExternalBg */:
                                    openExternal(params.srcURL, type ===
                                        1 /* ImageCallbackTypes.OpenExternalBg */);
                                    break;
                                case 2 /* ImageCallbackTypes.SaveAs */:
                                    contents.session.downloadURL(params.srcURL);
                                    break;
                                case 3 /* ImageCallbackTypes.Copy */:
                                    contents.copyImageAt(params.x, params.y);
                                    break;
                                case 4 /* ImageCallbackTypes.CopyLink */:
    ...
    electron_1.ipcMain.handle("init-font-list", () => {
