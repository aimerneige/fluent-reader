...
electron_1.ipcMain.on("get-view", event => {
    event.returnValue = exports.store.get(VIEW_STORE_KEY, 1 /* ViewType.List */);
});
...
electron_1.ipcMain.on("get-theme", event => {
    event.returnValue = exports.store.get(THEME_STORE_KEY, "system" /* ThemeSettings.Default */);
});
...
electron_1.ipcMain.on("get-font", event => {
    event.returnValue = exports.store.get(FONT_STORE_KEY, "");
});
electron_1.ipcMain.handle("set-font", (_, font) => {
    exports.store.set(FONT_STORE_KEY, font);
});
...
electron_1.ipcMain.on("get-search-engine", event => {
    event.returnValue = exports.store.get(SEARCH_ENGINE_STORE_KEY, 0 /* SearchEngines.Google */);
});
...
electron_1.ipcMain.on("get-view-configs", (event, view) => {
    switch (view) {
        case 1 /* ViewType.List */:
            event.returnValue = exports.store.get(LIST_CONFIGS_STORE_KEY, 1 /* ViewConfigs.ShowCover */);
            break;
        default:
...
electron_1.ipcMain.on("get-ai-config", event => {
    event.returnValue = exports.store.get(AI_CONFIG_STORE_KEY, null);
});
electron_1.ipcMain.handle("set-ai-config", (_, config) => {
    exports.store.set(AI_CONFIG_STORE_KEY, config);
});
const OPEN_TARGET_STORE_KEY = "openTarget";
electron_1.ipcMain.on("get-open-target", event => {
    event.returnValue = exports.store.get(OPEN_TARGET_STORE_KEY, 0);
});
electron_1.ipcMain.handle("set-open-target", (_, target) => {
    exports.store.set(OPEN_TARGET_STORE_KEY, target);
});
const LOW_PERFORMANCE_STORE_KEY = "lowPerformance";
electron_1.ipcMain.on("get-low-performance", event => {
    event.returnValue = exports.store.get(LOW_PERFORMANCE_STORE_KEY, false);
});
electron_1.ipcMain.handle("set-low-performance", (_, flag) => {
    exports.store.set(LOW_PERFORMANCE_STORE_KEY, flag);
});
const AGGRESSIVE_CACHE_STORE_KEY = "aggressiveCache";
electron_1.ipcMain.on("get-aggressive-cache", event => {
    event.returnValue = exports.store.get(AGGRESSIVE_CACHE_STORE_KEY, false);
});
electron_1.ipcMain.handle("set-aggressive-cache", (_, flag) => {
    exports.store.set(AGGRESSIVE_CACHE_STORE_KEY, flag);
});
