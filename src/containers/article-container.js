...
const getAIRequest = (state) => state.app.aiRequest;
const makeMapStateToProps = () => {
    return (0, reselect_1.createSelector)([getItem, getSource, getLocale, getAIRequest], (item, source, locale, aiRequest) => ({
        item: item,
        source: source,
        locale: locale,
        aiRequest: aiRequest,
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        clearAIRequest: () => dispatch((0, app_1.clearAIRequest)()),
        updateSourceTextDirection: (source, direction) => {
            dispatch((0, source_1.updateSource)({ ...source, textDir: direction }));
        },
    };
};
